...
if "GOOGLE_APPLICATION_CREDENTIALS_JSON" in os.environ:
    creds_content = os.environ["GOOGLE_APPLICATION_CREDENTIALS_JSON"]
    creds_dict = json.loads(creds_content)

    # Write creds into a temporary file
    creds_file = tempfile.NamedTemporaryFile(delete=False, suffix=".json")
    creds_file.write(json.dumps(creds_dict).encode())
    creds_file.flush()
...
app = FastAPI(
    title="lawScan API",
    description="Upload, analyze, chat with, and get audio explanations of legal documents using Vertex AI",
    version="1.0.0"
)
...
async def startup_event():
    ...
    try:
        print("🚀 Starting lawScan (empty mode).")
